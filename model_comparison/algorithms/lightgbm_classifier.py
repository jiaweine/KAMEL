"""
LightGBM implementation for ML comparison experiments.
"""
...
try:
    ...
    print("Warning: LightGBM not available. Please install with: pip install lightgbm")


class LightGBMClassifier_ML:
    """
    LightGBM with standard preprocessing and oversampling.
    """
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray, 
        ...
        pos_count = np.sum(y_train_proc == 1)
        neg_count = np.sum(y_train_proc == 0)
        scale_pos_weight = neg_count / pos_count if pos_count > 0 else 1.0
        
        # Initialize model with configuration parameters
        model_params = self.config.copy()
        model_params['random_state'] = self.random_state
        # Handle scale_pos_weight if set to 'auto'
